...
class DemoNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("Python Demo node started.")
    ...
    def topic_callback(self, msg):
        self.get_logger().info(f"Demo Received: {msg.data}")

        if not self.input_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn("Input service not available.")
            return
    ...
    def send_tts_request(self):
        ...
        request.input = "TTS:这个是我需要进行语音合成的文本"

        future = self.input_client.call_async(request)
        future.add_done_callback(self.input_response_callback)
    ...
    def send_volume_request(self):
        ...
        self.get_logger().info("Sent volume request")
    ...
    def send_wakeup_request(self):
        ...
        self.get_logger().info("Sent wakeup request")
    ...
    def input_response_callback(self, future):
        try:
            ...
            self.get_logger().info(
                f"Input Service response: {response.input_response}"
            )
