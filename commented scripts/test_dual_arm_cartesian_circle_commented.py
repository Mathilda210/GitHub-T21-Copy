...
# --------------------------- IMPORTS ---------------------------
# __future__ is a python module that allows you to use features 
# from future versions of Python in your current version.
...
# Custom interfaces for motion control messages and services
...
LEFT_ARM_IDX = 0
RIGHT_ARM_IDX = 1
# Pose size for Cartesian commands (x, y, z, qw, qx, qy, qz) ??
POSE_SIZE = 7
...
def _fmt_pose(pose: Sequence[float]) -> str:
    """Format a Cartesian pose as a string with 6 decimal places."""
...
def circle_pose_left(
    # Theta is the angle parameter for the circle
    theta: float,
    # x0, y0, z0 are the center coordinates of the circle
    x0: float,
    y0: float,
    z0: float,
    # qw, qx, qy, qz are the quaternion components for orientation
    qw: float,
    qx: float,
    qy: float,
    qz: float,
...
# --------------------------- DUAL ARM CIRCLE PUBLISHER NODE ---------------------------
class DualArmCirclePublisher(Node):
    def __init__(self, args: argparse.Namespace):
        super().__init__("dual_arm_cartesian_circle")

        # Takes the command line arguments and stores them in the instance variables
        # These arguments are optional parameters 
        self._args = args
        self._command_id = int(args.command_id)
        self._left_start = list(args.left_start_pose)

        # Creates a publisher for the /external/online_motion_command topic, 
        # which is used to send Cartesian motion commands to the robot
        ...
        # Creates a service client for the /external/pose_demo service,
        self._pose_demo_client = self.create_client(
            # service type (custom service defined in px_mc_msgs)
            PoseDemo,
            # service name
            "/external/pose_demo",
        )
    ...
    def call_go_home_pose_demo(self) -> bool:
        ...
        # Create a request for the PoseDemo service with the specified parameters
        ...
        request.speed_scale = float(self._args.pose_demo_speed_scale)
        request.repeat_count = int(self._args.pose_demo_repeat_count)
        request.timeout_sec = float(self._args.pose_demo_timeout_sec)
        ...
        # Send the request asynchronously to the service pose_demo_client
        # and wait for the response, with a timeout
        # NB: We called the service but we did not write it, it is most probably define within the robot's software 
    ...
    # Usually to call a method you do node.method()
    # Here because we put the @property the method can be called like an attribute 
    # Example: node.x0 instead of node.x0()
    @property
    def x0(self) -> float:
    ...
    def publish_dual_cartesian(
        ...
        # Create a message to send a Cartesian trajectory command for both arms
        ...
        # Creates two TrajectoryPoint messages, one for each arm, 
        # and appends them to the target_point list in the OnlineMotionCommand message.
        # NB: target_point[i] aligns with target_arm_indices[i]
        ...
        # Set the maximum velocity, acceleration, and jerk constraints for both arms 
        # using the values provided in the command line arguments.
        msg.max_velocity = [
            ArmConstraintParameters(
        ...
        # Set additional parameters for the OnlineMotionCommand message
        ...
        # Eventually publish the OnlineMotionCommand message 
        # to the /external/online_motion_command topic
        self._motion_pub.publish(msg)

        # Increment the command ID for the next command and return the current command ID
